...
const ResumeUpload: React.FC<ResumeUploadProps> = ({ onComplete, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError('Neural synthesis failed. The document might be too complex or the connection was interrupted. Please try again with a simpler format.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl animate-fade-in">
        <div className="relative w-full max-w-lg px-6 flex flex-col items-center">
          ...
          <h2 className="text-3xl font-black dark:text-white mb-4 text-center tracking-tighter">
            Sequencing Persona...
          </h2>
          ...
          <div className="h-6 flex items-center justify-center">
             <p className="text-indigo-600 dark:text-indigo-400 font-bold uppercase text-[10px] tracking-[0.3em] animate-pulse">
               {LOADING_STAGES[loadingStageIdx]}
             </p>
          </div>

          <div className="mt-16 grid grid-cols-4 gap-2 opacity-20">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="h-1 bg-indigo-500 rounded-full animate-pulse" style={{ animationDelay: `${i * 150}ms` }}></div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          {mode === 'upload' ? (
              ...
              <div className="flex flex-col items-center">
                ...
                {file ? (
                  <div className="space-y-2">
                    <p className="text-xl font-black text-gray-900 dark:text-white">{file.name}</p>
                    <p className="text-sm text-gray-400">{(file.size / 1024 / 1024).toFixed(2)} MB • Synthesis Ready</p>
                    <button 
                      type="button"
                  ...
                  <>
                    <p className="text-2xl font-black text-gray-900 dark:text-white mb-3 tracking-tight">Drop Source Material</p>
                    <p className="text-gray-400 font-medium mb-8">Support for high-fidelity PDF and DOCX</p>
                    <button 
                      type="button"
            ...
            <div className="space-y-4">
              <label htmlFor="resumeText" className="block text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest ml-1">
                Raw Resume Data
              </label>
              <textarea
                id="resumeText"
          ...
          {error && (
            <div className="bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 p-6 rounded-2xl flex items-center border border-red-100 dark:border-red-900/30 animate-pulse">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-4 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-12 px-6 text-center">
        <div>
          <h4 className="font-black text-gray-900 dark:text-white mb-3 uppercase text-[10px] tracking-widest text-indigo-500">Secure Protocol</h4>
          <p className="text-sm text-gray-500 dark:text-slate-400 font-medium leading-relaxed">Encrypted processing. Your legacy is only shared via your unique resume-ai.app link.</p>
        </div>
        <div>
          <h4 className="font-black text-gray-900 dark:text-white mb-3 uppercase text-[10px] tracking-widest text-indigo-500">Gemini 3 Flash</h4>
          <p className="text-sm text-gray-500 dark:text-slate-400 font-medium leading-relaxed">Ultra-low latency extraction engine designed for high-fidelity professional synthesis.</p>
        </div>
        <div>
          <h4 className="font-black text-gray-900 dark:text-white mb-3 uppercase text-[10px] tracking-widest text-indigo-500">Neural Sync</h4>
          <p className="text-sm text-gray-500 dark:text-slate-400 font-medium leading-relaxed">Instant conversion from static documents to interactive, conversational career agents.</p>
        </div>
      </div>
    </div>
  );
